...
// paths (so it loads from file://) and the PWA plugin disabled — no service
// worker, and `virtual:pwa-register` becomes a no-op (updates are handled by
// electron-updater, and SWs don't run under file://). The web build is
// unchanged and still targets the GitHub Pages base path.
export default defineConfig(({ command, mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      VitePWA({
        disable: isElectron,
        // 'prompt': a new service worker waits until the user opts in via the
        // update banner / version badge (see src/pwa), instead of reloading
        // mid-game.
        registerType: 'prompt',
